...
describe("Generator", function() {
  describe("#buildData", function() {
    it("should build microsite.liquid and microsite.css if when the developer wants microsite", function(){
      var g = new Generator();

      var manifest = g.buildData();
      assert.equal(1, manifest.files.microsite.length);
    });

    it("should build mobile.liquid and mobile.css when the developer wants mobile", function(){
      var g = new Generator();

      var manifest = g.buildData();
      assert.equal(1, manifest.files.mobile.length);
    });
